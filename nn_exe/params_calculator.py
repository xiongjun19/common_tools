...
def calc_gpt_params(layer_num, hid_dim, vocab_size=50000, ffn_mid=None):
    ...
    res = att_param + ffn_param + lm_head_param
    return res
...
if __name__ == '__main__':
    hid  = 1024
    layers = 24
    params = calc_gpt_params(layers, hid)
    print(params / 1e9)
    hid  = 4096
    layers = 32
    params = calc_gpt_params(layers, hid)
    ...
    moe_layers = 12
    num_exp = 128
    params = calc_gpt_moe_params(layers, hid, moe_layers, num_exp)
    print(params / 1e9)
    hid = 2048
    layers = 24
    moe_layers = 12
    ...
    params = calc_gpt_moe_params(layers, hid, moe_layers, num_exp)
    print(params / 1e9)
    print("now calc  swtich-xxl param:")
    enc_layer = 24
    moe_enc_layer = 12
    dec_layer = 24
    moe_dec_layer = 12
    num_exp = 128
    hid = 4096
    params = calc_t5_param_moe(enc_layer, dec_layer,
                                        hid, moe_enc_layer,
                                        moe_dec_layer, num_exp, ffn_mid=10240)
                                        # moe_dec_layer, num_exp)

    print(params / 1e9)
    params = calc_t5_param(enc_layer, dec_layer,
                           hid, ffn_mid=10240)
