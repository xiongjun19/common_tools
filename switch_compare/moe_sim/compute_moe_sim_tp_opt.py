...
def _calc_moe_info(bs, micro_bs, seq_len, hid_dim,
    ...
    comm_time = times * 2 * (tp -1) * comm_size / (bandwidth * tp) * 1000
...
def _calc_comp_and_buble(tot_batch, micro_batch, moe, tp,  pp, dp, seq,
    ...
    act_layers = tot_layers // pp
    compute_flops = calc_micro_comp_flops(micro_batch, tp, moe,  seq, h_dim, mid_dim, act_layers)
...
def calc_micro_comp_flops(micro_batch, tp, moe, seq, h_dim, mid_dim, act_layers):
    att = 4 * h_dim * h_dim * seq * micro_batch * 2 + \
             + micro_batch * h_dim * seq * seq * 2
    ffn = 2 * 3 * h_dim * mid_dim * seq * micro_batch * 2
    res = 3 * (att / tp + ffn / tp) * act_layers
    return res
...
def _visual_res(df, out_file):
    ...
    global_bs_arr = [16, 32, 64, 128, 256, 512, 2048, 1344, 2688, 5376]
