...
def _calc_comp_and_tp_info(job, no_overlap):
    ...
    pp = job.train_cfg.pp
    cp = job.train_cfg.cp
    one_flop_info = _calc_one_layer_flops(job, cp, tp)
    ...
    tp_msg, tp_times = _calc_one_layer_tp(job, cp)
    ...
    return comp_time, buble_time, tp_comm_time
...
def _calc_one_layer_flops(job, cp, tp):
    bs = job.train_cfg.micro_bs
    seq = job.train_cfg.seq_len
    seq = seq / cp
    h = job.model_cfg.hid_dim
...
def _compute_impl(job, item, no_overlap):
    new_item = item.copy()
    comp_time, buble_time, tp_comm_time = _calc_comp_and_tp_info(job, no_overlap)
