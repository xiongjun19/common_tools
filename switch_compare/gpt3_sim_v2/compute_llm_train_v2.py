...
def _calc_comp_and_tp_info(job, no_overlap):
    ...
    one_flop_info = _calc_one_layer_flops(job)
    ...
    tp = job.train_cfg.tp
    pp = job.train_cfg.pp
    tf_tb_one_chunk = chunk_layers * one_flop_info['tot'] / (peak_flops *1e12* comp_util) / tp
    bs = job.train_cfg.micro_bs
    dp = job.train_cfg.dp
    pp = job.train_cfg.pp
    ...
    tp_msg, tp_times = _calc_one_layer_tp(job)
