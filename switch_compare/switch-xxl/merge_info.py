...
def _merge_info(input_items, comm_items):
    ...
    for topo in range(1, 4):
        for item in input_items:
            new_item = item.copy()
            moe_time = _get_moe_lat(new_item, comm_items, topo)
            pp_time = _get_pp_lat(new_item, comm_items, topo)
            dp_time = _get_dp_lat(new_item, comm_items, topo)
            overall_time = moe_time + pp_time + dp_time + new_item['tot_time']
            new_item['overall_time'] = overall_time
            new_item['moe_comm_time'] = moe_time
...
def _get_moe_lat(new_item, comm_items, topo):
    tp_lat = _query_lat(new_item, comm_items, topo, 'MOE')
    return tp_lat
