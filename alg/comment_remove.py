...
class Remover(object):
    ...
    def removeComments(self, source):
        ...
        """
        res = []
        state = self.NORM_STATE
        pre_state = self.NORM_STATE
        tmp_str = ""
        for str_ in source:
            cl_str, state = self._clean_str(state, str_)
            if state == self.NORM_STATE:
                if pre_state == self.BLOCK_STATE:
                    tmp_str += cl_str
                if len(tmp_str) > 0:
                    res.append(tmp_str)
                    tmp_str = ""
    ...
    def _clean_str(self, state, text):
        start = 0
        to_remove_arr = []
        end_taken = set()
        for i, ch in enumerate(text):
            if state == self.LOCAL_STATE:
                to_remove_arr.append([start, len(text) - 1])
                return self._clean_str_by_pos_arr(text, to_remove_arr), self.NORM_STATE
            elif state == self.BLOCK_STATE:
                if ch == r'/' and i > 0 and text[i-1] == r'*' and (i-1) not in end_taken:
                    end = i
                    to_remove_arr.append([start, end])
                    end_taken.add(end)
                    state = self.NORM_STATE
            else:
                if ch == r'/' and i > 0 and text[i-1] == r'/' and i-1 not in end_taken:
                ...
                elif ch == r'*' and i > 0 and text[i-1] == r'/':
                    start = i - 1
                    end_taken.add(i)
        ...
        if state != self.NORM_STATE:
            ...
            if state == self.LOCAL_STATE:
                state = self.NORM_STATE
        return self._clean_str_by_pos_arr(text, to_remove_arr), state

    def _clean_str_by_pos_arr(self, text, pos_arr):
        if not pos_arr:
            return text
        res = []
        for i, ch in enumerate(text):
            if any([begin <= i <= end for begin, end in pos_arr]):
                continue
            res.append(ch)
        return "".join(res)
...
if __name__ == "__main__":
    test_str = ["/*Test program */", "int main()", "{ ", "  // variable declaration ", "int a, b, c;", "/* This is a test", "   multiline  ", "   comment for ", "   testing */", "a = b + c;", "}"]
    # test_str = ["a/*comment", "line", "more_comment*/b"]
    test_str = [
        "struct Node{",
        "    /*/ declare members;/**/",
        "    int size;",
...
"dbbdedece//*dceaa//*aeb/*/bdead//*ccbeb*////**//b//*d*//aecad//*a*////**//abeb"
    ...
    t_obj = Remover()
    t_res = t_obj.removeComments(test_str)
    print(t_res)
